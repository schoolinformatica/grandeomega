...
{
    /*
     * When having enough time, implement this class into the data mining aspect. 
    ...
    public static class DES
    {
        ...
        public static double[] ForeCast(double[] values, int start, int end, double alpha, double beta)
        {
            ...
            var n = values.Length;
            var forecast = new double[n + end - start + 1];
            Smooth(values, alpha, beta).CopyTo(forecast, 0);
            var Sn = CalculateSmoothing(values, alpha, beta, n - 1);
            var Bn = CalculateBeta(beta, alpha, values, n - 1);
        ...
        private static double SquaredError(double[] points, double alpha, double beta)
        {
            var se = points
                         ...
                         .Select(
                             (x, i) => Math.Pow(
                                 Math.Abs(
                                     x - (CalculateSmoothing(points, alpha, beta, (i + 2) - 1) -
                                          CalculateBeta(beta, alpha, points, (i + 2) - 1))
                                 ),
                                 2)
                         )
                         .Sum() / (points.Length - 2);
